...
//     return filename;
// }
function readCSVFile(filePath, column1, column2, callback) {
...
app.use('/', upload.single('inputFile'), function (req, res) {
    ...
    if(data_csv[filenameWithoutExtension]){
        res.send(`${filenameWithoutExtension}` + ':' + `${data_csv[filenameWithoutExtension]}`)
    }
